...
export const SnakeArcade = () => {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <div className="mx-auto grid min-h-screen w-full max-w-7xl gap-5 px-4 py-5 lg:grid-cols-[330px_minmax(420px,1fr)_330px] lg:px-6">
        <aside className="space-y-4 lg:pt-8">
          <div className="rounded-lg border border-panel-border bg-panel/90 p-5 shadow-panel backdrop-blur">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">Play locally now. Auth, ranking, and watching all route through one mocked backend service.</p>
          </div>
          <AuthPanel user={user} onUser={setUser} />
          <section className="rounded-lg border border-panel-border bg-panel/90 p-4 shadow-panel backdrop-blur">
            <div className="flex items-center gap-2 text-panel-foreground"><Crown className="size-4 text-accent" /><h2 className="font-display text-2xl">Leaderboard</h2></div>
            <div className="mt-3 space-y-2">
              {leaderboard.map((entry, index) => (
                <div key={entry.id} className="grid grid-cols-[2rem_1fr_auto] items-center gap-2 rounded-md bg-secondary px-3 py-2 text-sm">
        ...
        <section className="flex flex-col justify-center gap-4 lg:py-8">
        ...
        <aside className="space-y-4 lg:pt-8">
          <section className="rounded-lg border border-panel-border bg-panel/90 p-4 shadow-panel backdrop-blur">
            <div className="flex items-center gap-2 text-panel-foreground"><Eye className="size-4 text-accent" /><h2 className="font-display text-2xl">Watch live</h2></div>
            <div className="mt-3 grid gap-2">
              {players.map((player) => (
